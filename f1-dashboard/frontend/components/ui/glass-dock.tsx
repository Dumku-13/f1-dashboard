                    ...
                    {items.map((el, index) => {
                        ...
                        return (
                            <div
                                ...
                                role="button"
                                // The icons are bare SVGs, so without this every
                                // dock item is an unnamed button to a screen reader
                                // (the only label lives in a hover-only tooltip).
                                aria-label={el.title}
                                tabIndex={0}
                                onKeyDown={(e) => {
                                ...
                                >
                                    {isAnimated ? (
                                        <MorphingIcon
                                            type={type}
                                            isActive={isActive}
                                            // MorphingIcon renders a real <button>. Giving it
                                            // handleClick too fired the handler twice per click
                                            // (button + bubbling to the wrapper), pushing two
                                            // history entries so Back needed two presses.
                                            // The wrapper above owns the click.
                                            onClick={() => { }}
